...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ─────────────────────────────────────────────────────────
    // FEATURE · INTERACTIVE JS CALENDAR
    // Built entirely in JS — appended to calendar section
    // ─────────────────────────────────────────────────────────
    const monthNames = [
    ...
    const calEventsByMonth = {};
    allHackathons.forEach(h => {
        if (!calEventsByMonth[h.month]) calEventsByMonth[h.month] = [];
        for (let d = h.startDay; d <= h.endDay; d++) {
            calEventsByMonth[h.month].push({
                day: d, name: h.name, mode: h.mode,
                prize: h.prize, date: h.date
            });
        }
    });

    let calCurrentMonth = 3; // April
    const calCurrentYear = 2026;

    function buildJSCalendar() {
        if (!calSection) return;
        const wrapper = document.createElement("div");
        wrapper.id = "js-calendar-wrapper";
        wrapper.innerHTML = `
            <h3>📅 Interactive Calendar</h3>
            <p class="js-cal-sub">Hover over highlighted days to see hackathons on that date.</p>
            <div id="js-calendar-controls">
                <button id="jsc-prev" aria-label="Previous month">‹</button>
                <span id="js-calendar-month-label"></span>
                <button id="jsc-next" aria-label="Next month">›</button>
            </div>
            <div id="js-calendar-grid"></div>
        `;
        const tooltip = document.createElement("div");
        tooltip.id = "jsc-tooltip";
        document.body.appendChild(tooltip);

        const container = calSection.querySelector(".container");
        if (container) container.appendChild(wrapper);

        document.getElementById("jsc-prev").addEventListener("click", () => {
            calCurrentMonth = (calCurrentMonth - 1 + 12) % 12;
            renderJSCalendar();
        });
        document.getElementById("jsc-next").addEventListener("click", () => {
            calCurrentMonth = (calCurrentMonth + 1) % 12;
            renderJSCalendar();
        });
    ...
    function renderJSCalendar() {
        const grid = document.getElementById("js-calendar-grid");
        const label = document.getElementById("js-calendar-month-label");
        ...
        const events = calEventsByMonth[calCurrentMonth] || [];

        for (let i = 0; i < firstDay; i++) {
            const c = document.createElement("div");
            c.className = "jsc-cell empty";
            grid.appendChild(c);
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            cell.className = "jsc-cell";

            const dayEvents = events.filter(e => e.day === day);
            if (dayEvents.length > 0) {
                cell.classList.add("has-event");
                cell.classList.add(dayEvents[0].mode);

                const dot = document.createElement("span");
                dot.className = "jsc-event-dot";
                cell.appendChild(dot);

                cell.addEventListener("mouseenter", () => {
                    const tip = document.getElementById("jsc-tooltip");
                    if (!tip) return;
                    tip.innerHTML = dayEvents.map(e => `
                        <div class="tip-title">${e.name}</div>
                        <div class="tip-meta">${e.date} · ${e.mode} · $${e.prize.toLocaleString()}</div>
                    `).join('<hr style="border:0;border-top:1px solid #222;margin:.4rem 0;">');
                ...
                cell.addEventListener("click", () => {
                    showToast(`${dayEvents[0].name} — ${dayEvents[0].date}`, "📅");
                });
            }
